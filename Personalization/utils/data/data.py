...
class SPEECHCOMMANDS_truncated(data.Dataset):
    def __init__(self, root, dataidxs=None, train=True, transform=None, target_transform=None, download=False):
        self.root = root
        self.dataidxs = dataidxs
        self.train = train
    ...
    def __build_truncated_dataset__(self):
        if os.path.isfile(os.path.join(self.root, 'speechcommands/train_data.pt')) == False:
            from utils.data.data_downloader import save_data_speechcommands
            save_data_speechcommands(self.root)
        if self.train:
            data = torch.load(os.path.join(self.root, 'speechcommands/train_data.pt'))
            target = np.load(os.path.join(self.root, 'speechcommands/train_target.npy'))
        else:
            data = torch.load(os.path.join(self.root, 'speechcommands/test_data.pt'))
            target = np.load(os.path.join(self.root, 'speechcommands/test_target.npy'))
        if self.dataidxs is not None:    
                # Need to pad the elements to get tensors of similar lengths 
                import torch.nn.functional as F
                max_length = max(t.shape[1] for t in data)
                padded_tensors = []
                for t in data:
                    padded = F.pad(t, (0, max_length - t.shape[1]))
                    padded_tensors.append(padded)
                data = torch.stack(padded_tensors)

                # This is a 1D tensor, so we need to use a loop or a list comprehension to select multiple elements
                data = torch.stack([data[i] for i in self.dataidxs])
                target = torch.stack([torch.tensor(target[i]) for i in self.dataidxs])
                target = nn.functional.one_hot(target, num_classes=35)
        return data, target #the problem is in the target shape 
...
def get_dataloader(dataset, datadir, train_bs, test_bs, train_dataidxs=None, test_dataidxs=None):
    if dataset == 'cifar10':
        ...
        dl_obj = SPEECHCOMMANDS_truncated
        transform_train = transforms.Compose([
            transforms.ToTensor()
        ])
        transform_test = transforms.Compose([
...
def partition_data(args, logger):
    if args.dataset == 'cifar10':
        ...
        y_train = SPEECHCOMMANDS_truncated(args.datadir, train=True, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 35
    ...
    with open(f'{args.logdir}/train_data_statistics_{args.init_seed}.pkl', 'wb') as f:
        pickle.dump(net_cls_counts, f)
    return net_dataidx_map

def test_partition_data(args, logger):
    if args.dataset == 'cifar10':
        y_test = CIFAR10_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 10
    elif args.dataset == 'fmnist':
        y_test = FashionMNIST_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 10
    elif args.dataset == 'kmnist':
        y_test = KMNIST_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 10  
    elif args.dataset == 'speechcommands': 
        y_test = SPEECHCOMMANDS_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 35
    elif args.dataset == 'cifar100':
        y_test = CIFAR100_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 100
    elif args.dataset == 'svhn':
        y_test = SVHN_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 10
    elif args.dataset == 'covertype':
        y_test = COVERTYPE_truncated(args.datadir, train=False, download=True, transform=transforms.Compose([transforms.ToTensor()])).target
        K = 7
    else:
        assert NotImplementedError(f"{args.dataset} is not available!")
    ...
    logger.info(f'Test data statistics: {net_cls_counts}')
    return net_dataidx_map

def data_handler(args, logger):
    logger.info("Partitioning data")
    net_dataidx_map = partition_data(args, logger)
    test_net_dataidx_map = test_partition_data(args, logger)
    _, test_dl_global = get_dataloader(args.dataset, args.datadir, args.batch_size, 32)   
    with open(f'{args.logdir}/test_dl_global_{args.init_seed}.pkl', 'wb') as f:
        pickle.dump(test_dl_global, f)                                                
    return test_dl_global, net_dataidx_map , test_net_dataidx_map