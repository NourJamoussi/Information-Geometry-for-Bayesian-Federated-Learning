...
def save_data_speechcommands(datadir): #TODO: Fix this
    os.makedirs(os.path.join(datadir, 'speechcommands'), exist_ok=True)
    class SubsetSC(SPEECHCOMMANDS):
        def __init__(self, subset: str = None):
            super().__init__(os.path.join(datadir, 'speechcommands'), download=True)
    ...
    train_data = [datapoint[0] for datapoint in train_set] # the waveforms 
    train_target = np.array([label_to_index(datapoint[2]) for datapoint in train_set]) # the label 
    torch.save(train_data, os.path.join(datadir, 'speechcommands/train_data.pt'))
    np.save(os.path.join(datadir, 'speechcommands/train_target.npy'), train_target)

    test_data = [datapoint[0] for datapoint in test_set] # the waveform
    test_target = np.array([label_to_index(datapoint[2]) for datapoint in test_set]) # the label 
    torch.save(test_data, os.path.join(datadir, 'speechcommands/test_data.pt'))
    np.save(os.path.join(datadir, 'speechcommands/test_target.npy'), test_target)
